...
namespace Ch06DemosTemplate {
    internal class Program {

        // no params and no return - calling a method to print to the console
        private static void PrintHello() {
            Console.WriteLine("Hello, form PrintHello method.");
        }

        // one param and no return
        private static void PrintHello(string name = "No Name") {
            Console.WriteLine($"Hello, {name}. Welcome to our application!");
        }

        // one param and return a int
        // prompt the user for a whole number
        // Ch 07 - Exception Handling!!! (p. 207)
        private static int PromptInt(string prompt) {
            int wholeNumber = 0;
            // using while true to break out
            while (true) {
                try {
        ...
        private static int PromptPositiveInt(string prompt)
        {
            ...
            while (true)
            {
                ...
                if (wholeNumber < 0)
                {
                    Console.WriteLine("Error: number mush be 0 or greater");
                }
                else                
                    break;
            }
        ...
        private static int PromptInt(string prompt, int minVal, int maxVal)
        {
            int wholeNumber = 0;
        ...
        static void Main(string[] args)
        {
            ...
            // prompt for positive intergers, within 1 to 100range
            ...
            int dayNumber = PromptInt("Endter day of week number: ", 1, 7);
            int dayOfYear = PromptInt("Endter day of the year: ", 1, 365);
            ...
            string title = "Comedy Movie";
            int year = 2020;
            string rating = "PG";
            string director = "Brendan Keam";
